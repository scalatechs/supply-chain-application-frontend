        ...
        <div className='w-full space-y-8 overysc'>
            ...
            <div className="grid gap-4 xl:grid-cols-4 lg:grid-cols-3">
                <StatsCard title="Total Inventory"
                    value={`${data?.data?.totalInventory.toLocaleString() == undefined ? "0.00" : data?.data?.totalInventory.toLocaleString()}`}
                    icon={''}
                    currency="Rs " />
                <StatsCard title="Units on Hand"
                    value={`${data?.data?.totalUnitsOnHand == undefined ? "0" : data?.data?.totalUnitsOnHand}`}
                    icon={''} />
                <StatsCard title="Units on Order"
                    value={`${data?.data?.unitsOnOrder == undefined ? "0" : data?.data?.unitsOnOrder}`}
                    icon={''} />
                <StatsCard title="Units to Reorder"
                    value={`${data?.data?.unitToreorder.length == undefined ? "0" : data?.data?.unitToreorder.length}`}
                    icon={''} />
            </div>
